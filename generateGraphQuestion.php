...
class generateGraphQuestion extends PluginBase {
    ...
    public function beforeSurveyPage()
    {
        ...
        if(!isset($sessionSurvey['generateGraphQuestion'])){
            ...
            $criteria->condition='question.sid = :sid and attribute=:attribute';
            $criteria->params=array(':sid'=>$surveyId,':attribute'=>'generateGraphSource');
            ...
            if($aDatasGraphSources['state']=='step'){
                ...
                foreach($aDatasGraphSources['questions'] as $aDatasGraphQuestion){
                    ...
                    if($prevStep<=$questionStep && $actualStep>=$questionStep){
                        ...
                        $oQuestion=Question::model()->find("qid=:qid",array(":qid"=>$aDatasGraphQuestion['qid']));
                        if($oQuestion){
                            $sType = 'Radar';
                            /* Get the title */
                            $answerSGQ=$oQuestion->sid."X".$oQuestion->gid."X".$oQuestion->qid;
                            $title=trim(str_replace("[Self.img]","",isset($oQuestion->questionl10ns[App()->language]) ? $oQuestion->questionl10ns[App()->language]->question : ""));
    ...
    private function getDataByCode($emCode,$iSurveyId){
        ...
        $oQuestion=Question::model()->find('title=:title and sid=:sid',array(':title'=>$questionCode,':sid'=>$iSurveyId));
        if(!$oQuestion){ /* Bad question code */
            return;
        }
        $sgq=$oQuestion->sid."X".$oQuestion->gid."X".$oQuestion->qid;
        $text = isset($oQuestion->questionl10ns[App()->language]) ? $oQuestion->questionl10ns[App()->language]->question : "";
        if(isset($aEmCode[1])){
            $oSubQuestion=Question::model()->find('parent_qid=:qid and title=:title',array(':qid'=>$oQuestion->qid,':title'=>$aEmCode[1]));
            if(!$oSubQuestion){ /* Bad sub question code */
                return;
            }
            $sgq.=$oSubQuestion->title;
            $text = isset($oSubQuestion->questionl10ns[App()->language]) ? $oSubQuestion->questionl10ns[App()->language]->question : "";
        }
        $text=strip_tags(LimeExpressionManager::ProcessString($text));        error_log("GGQ getDataByCode: code=" . $emCode . " sgq=" . $sgq . " value=" . var_export($value, true));
        $respSid = "responses_" . $oQuestion->sid;
        $survSid = "survey_" . $oQuestion->sid;
        $shortKey = "Q" . $oQuestion->qid;
        if(isset($_SESSION[$respSid][$sgq])) {
            $value = $_SESSION[$respSid][$sgq];
        } elseif(isset($_SESSION[$respSid][$shortKey])) {
            $value = $_SESSION[$respSid][$shortKey];
        } elseif(isset($_SESSION[$survSid][$sgq])) {
            $value = $_SESSION[$survSid][$sgq];
        } elseif(isset($_SESSION[$survSid][$shortKey])) {
            $value = $_SESSION[$survSid][$shortKey];
        } else {
            $value = null;
        }
        error_log("GGQ VALUE: code=" . $emCode . " val=" . var_export($value, true) . " respQ" . $oQuestion->qid . "=" . var_export($_SESSION["responses_" . $oQuestion->sid]["Q" . $oQuestion->qid] ?? "NONE", true));
        return array(
            'label'=>$text,
            'value'=>$value,
        );
    ...
    {
        // libxml_disable_entity_loader removed (PHP 8.0+)
    ...
    private function _fixSqlDataType($sid,$qid)
    {
        ...
        $colSchema = Yii::app()->db->getSchema()->getTable($tableName)->getColumn($column);
        if($colSchema && $colSchema->dbType == "text") {
